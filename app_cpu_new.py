...
def font_pair_tzi(file_path1,file_path2):
    ...
    file_tail1=os.path.splitext(filename1)[1]
    file_tail2=os.path.splitext(filename2)[1]
    if file_tail1 in zip_tail_list and not uzip(file_path1,reference_dir,[]):
        return redirect(url_for('error'))
    if file_tail2 in zip_tail_list and not uzip(file_path2,source_dir,[]):
        return redirect(url_for('error'))
    ...
    if file_tail1.lower() in imgfile_tail_list:
        new_path1=os.path.join(reference_dir,"1",filename1)
        makedir(new_path1,True)
        os.rename(file_path1,new_path1)
    if file_tail2.lower() in imgfile_tail_list:
        new_path2=os.path.join(source_dir,"2",filename2)
        makedir(new_path2,True)
        os.rename(file_path2,new_path2)

    # ocr(root_path,os.path.join(source_dir,os.listdir(source_dir)[0]))
    if not os.listdir(os.path.join(source_dir,os.listdir(source_dir)[0])) or not os.listdir(os.path.join(reference_dir,os.listdir(reference_dir)[0])):return redirect(url_for('error'))
...
def font_upload1():
    if request.method == 'POST':
        ...
        global ERROR_FLAG
        _,file_tail1=os.path.splitext(f1.filename)
        _,file_tail2=os.path.splitext(f2.filename)
        ERROR_FLAG=False
        zip_tail_list=[".zip"]
        file_tail_list=[".ttf",".otf"]
        imgfile_tail_list=[".png",".jpg","jpeg",".bmp"]
        if os.path.exists(reference_dir):shutil.rmtree(reference_dir)
        else:makedir(reference_dir)
        if os.path.exists(source_dir):shutil.rmtree(source_dir) 
        else:makedir(source_dir)
        if (file_tail1.lower() in file_tail_list or file_tail1.lower() in imgfile_tail_list or file_tail1.lower() in zip_tail_list)and (file_tail2.lower() in file_tail_list or file_tail2.lower() in imgfile_tail_list or file_tail2.lower() in zip_tail_list):accurate,similari_list,ph,ah,dh,ssim,psnr=font_pair_tzi(upload_file_path1,upload_file_path2)
        else:return redirect(url_for('error'))
        if accurate:return "神经网络相似程度为{},感知哈希相似程度为{},均值哈希相似程度为{},差值感知相似程度为{},ssim相似程度为{},psnr相似程度为{}".format(str(accurate),str(ph[0]),str(ah[0]),str(dh[0]),str(ssim[0]),str(psnr[0]))
            # return json.dumps({'file_id': similari_list, 'filename': ph[0] , 'links_to' : ah})
        else:return "没有在字库中找到相应的汉字"
    else:return redirect(url_for('index'))
